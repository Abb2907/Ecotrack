...
def test_calculator_api_endpoints(client: TestClient) -> None:
    ...
    response = client.post("/api/v1/calculator/transport", json={"distance_km": 50.0, "mode": "gasoline_car"})
    assert response.status_code == 200
    assert response.json() == {"co2_kg": 9.6}
    
    # 2. Test energy calculation endpoint
    response = client.post("/api/v1/calculator/energy", json={"kwh": 100.0, "source": "grid_electricity"})
    assert response.status_code == 200
    assert response.json() == {"co2_kg": 37.1}
    
    # 3. Test diet calculation endpoint
    response = client.post("/api/v1/calculator/diet", json={"meals": 3, "diet_type": "vegan"})
    assert response.status_code == 200
...
def test_calculator_api_invalid_inputs(client: TestClient) -> None:
    ...
    response = client.post("/api/v1/calculator/transport", json={"distance_km": -10.0, "mode": "gasoline_car"})
