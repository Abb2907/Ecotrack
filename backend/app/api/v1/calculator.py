...
class TransportCalcRequest(BaseModel):
    ...
    mode: str = Field(..., description="gasoline_car | diesel_car | electric_car | bus | train")
...
async def calculate_transport(
    ...
    co2 = await emission_client.calculate_transport_emissions(payload.distance_km, payload.mode)
...
async def calculate_diet(
    ...
    co2 = await emission_client.calculate_diet_emissions(payload.meals, payload.diet_type)
    return CalcResponse(co2_kg=co2)
