...
async def export_my_data(
    ...
    logger.info(
        f"DATA_EXPORT_REQUEST: id={req_id} user={uid} timestamp={datetime.utcnow().isoformat()}"
    )

    data = await user_repo.export_user_data(uid)
    if not data:
        logger.warning(
            f"DATA_EXPORT_FAILURE: id={req_id} user={uid} reason='No user data found'"
        )
        raise HTTPException(
...
async def schedule_account_deletion(
    ...
    req_id = str(uuid.uuid4())
    logger.info(
        f"DELETE_SCHEDULE_REQUEST: id={req_id} user={uid} timestamp={datetime.utcnow().isoformat()}"
    )
    ...
    try:
        ...
        logger.info(
            f"DELETE_SCHEDULE_SUCCESS: id={req_id} user={uid} purge_date={purge_date.isoformat()}"
        )
        ...
        logger.error(
            f"DELETE_SCHEDULE_FAILURE: id={req_id} user={uid} reason='{str(e)}'"
        )
        raise
...
async def cancel_account_deletion(
    ...
    req_id = str(uuid.uuid4())
    logger.info(
        f"DELETE_CANCEL_REQUEST: id={req_id} user={uid} timestamp={datetime.utcnow().isoformat()}"
    )

    user = await user_repo.get_user(uid)
    if not user or not user.deletionRequested:
        logger.warning(
            f"DELETE_CANCEL_FAILURE: id={req_id} user={uid} reason='No active deletion request'"
        )
        raise HTTPException(
