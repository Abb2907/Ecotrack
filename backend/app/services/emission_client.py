...
class MockEmissionClient(BaseEmissionClient):
    ...
    async def calculate_diet_emissions(self, meals: int, diet_type: str) -> float:
        ...
        factors = {"meat_heavy": 3.2, "vegetarian": 1.2, "vegan": 0.7}
...
class ClimatiqEmissionClient(BaseEmissionClient):
    ...
    async def calculate_transport_emissions(
        ...
        activity_id = mapping.get(
            mode, "passenger_vehicle-vehicle_type_car-fuel_source_petrol-engine_size_na"
        )
    ...
    async def calculate_energy_emissions(self, kwh: float, source: str) -> float:
        activity_id = (
            ...
            if source == "grid_electricity"
            else "fuel-energy_source_natural_gas"
        )
        async with httpx.AsyncClient() as client:
            try:
                response = await client.post(
...
def get_emission_client() -> BaseEmissionClient:
    ...
    if (
        ...
        and climatiq_key != "gen-lang-client-0849880096"
    ):
