      ...
      <div className="flex flex-col items-center justify-center py-12 md:py-20 text-center">
        ...
        <div className="max-w-3xl px-4 space-y-6">
          ...
          <p className="text-lg md:text-xl text-brand-muted max-w-2xl mx-auto font-medium">
            EcoTrack connects with certified carbon models and Vertex AI to give you real-time
            feedback, daily tracking actions, and smart recommendations.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full mt-20 px-4">
          <Card>
            ...
            <p className="text-sm text-brand-muted">
              Instantly record conscious actions like cycling, eating vegan meals, and conserving
              power to track exact reduction values.
            </p>
          </Card>
          ...
          <Card>
            ...
            <p className="text-sm text-brand-muted">
              Get customized weekly suggestions generated by Google Gemini using your baseline data
              and active log trends.
            </p>
          </Card>
          ...
          <Card>
            ...
            <p className="text-sm text-brand-muted">
              Full data transparency. Export all profile activity logs or trigger safe deletion
              requests with a 7-day recovery grace period.
            </p>
          </Card>
        </div>
      </div>
      ...
      {profile?.deletionRequested && (
        <div className="flex items-center gap-3 p-4 bg-brand-error/10 border border-brand-error/30 rounded-xl text-brand-error">
          ...
          <div className="text-sm">
            <span className="font-bold">Account Scheduled for Deletion:</span> Your account and
            telemetry logs will be permanently deleted. You can cancel this request anytime on the{" "}
            <Link
              href="/privacy"
              className="underline font-semibold hover:text-brand-text transition-colors"
            >
              Privacy Settings
            </Link>{" "}
            page.
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="flex flex-col justify-between">
          <div>
            <span className="text-xs font-bold text-brand-muted uppercase tracking-wider">
              Carbon Baseline Target
            </span>
            <div className="text-3xl font-bold text-brand-text mt-1">
              {totalBaseline.toFixed(1)}{" "}
              <span className="text-sm text-brand-muted">kg CO2e/day</span>
            </div>
          ...
          <div className="mt-6 border-t border-brand-border/60 pt-4 flex gap-4 text-xs text-brand-muted">
            <div className="flex items-center gap-1">
              <Car className="h-3.5 w-3.5 text-brand-secondary" />{" "}
              {profile?.carbonBaseline.transport.toFixed(1)}
            </div>
            <div className="flex items-center gap-1">
              <Zap className="h-3.5 w-3.5 text-brand-accent" />{" "}
              {profile?.carbonBaseline.energy.toFixed(1)}
            </div>
            <div className="flex items-center gap-1">
              <Utensils className="h-3.5 w-3.5 text-brand-primary" />{" "}
              {profile?.carbonBaseline.diet.toFixed(1)}
            </div>
          </div>
        </Card>
        ...
        <Card className="flex flex-col justify-between">
          <div>
            ...
            <div className="text-3xl font-bold text-brand-primary mt-1">
              -{totalReduced.toFixed(1)} <span className="text-sm text-brand-muted">kg CO2e</span>
            </div>
            <p className="text-xs text-brand-muted mt-2">
        ...
        <Card className="flex flex-col justify-between">
          <div>
            ...
            <div className="text-3xl font-bold text-brand-secondary mt-1">
              ...
              <table className="w-full text-sm text-left text-brand-muted">
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr
                      key={log.logId}
                      className="border-b border-brand-border/30 hover:bg-slate-800/10"
                    >
        ...
        <Card hoverable={false} className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-sm text-brand-muted leading-relaxed">
              Google Gemini generates weekly actionable reduction ideas using your logged activities
              and baseline habits.
            </p>
            <div className="mt-4 p-4 rounded-xl bg-slate-800/40 border border-slate-700/50 text-xs text-brand-muted space-y-2">
              ...
              <p>
                Explore target recommendations around transportation optimizations, energy
                reductions, and daily actions.
              </p>
            </div>
          </div>
